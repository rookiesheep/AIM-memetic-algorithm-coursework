...
struct problem_struct** load_problems(const char *filename)
{
    ...
    //read the given number of variables , caonstraints and whether there is optimal solution value
...
void cross_over(struct solution_struct* curt_pop, struct solution_struct* new_pop) {
    for(int p = 0; p < POP_SIZE/2; p++) {
        ...
        if(crossover_rate < CROSSOVER_RATE){
            ...
            //the first node to do crossover operation
            ...
            //the second node to do crossover operation
...
//what if there it is already the best solution? no node in the chromosome would be changed and skip the evaluation
...
void replacement(struct solution_struct* curt_pop, struct solution_struct* new_pop)
{
    ...
    //add all individuals to rep_pop
    for(i = 0; i < POP_SIZE; i++) {
...
int main(int argc, const char * argv[]){
    ...
    for(int k=0; k<num_of_problems; k++) {
        ...
        output_solution(&best_sln, data_file);
